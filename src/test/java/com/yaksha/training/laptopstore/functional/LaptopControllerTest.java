...
public class LaptopControllerTest {
	...
	public void setup() throws Exception {
		...
		this.mockMvc = MockMvcBuilders.standaloneSetup(laptopController)
	...
	public void testControllerListLaptopsHome() throws Exception {
		try {
			...
			Page<Laptop> laptopPage = new PageImpl<>(expected);
			when(laptopService.getLaptops(pageable)).thenReturn(laptopPage);
	...
	public void testControllerSearchLaptops() throws Exception {
		...
		Pageable pageable = PageRequest.of(0, 5);
		Page<Laptop> laptopPage = new PageImpl<>(expected);
		when(laptopService.searchLaptops(key, pageable)).thenReturn(laptopPage);
	...
	@Test
	public void testControllerSearchLaptopsWithNullKey() throws Exception {
		List<Laptop> expected = getLaptopList(5);
		Pageable pageable = PageRequest.of(0, 5);
		Page<Laptop> laptopPage = new PageImpl<>(expected);
		when(laptopService.searchLaptops(null, pageable)).thenReturn(laptopPage);
	...
	@Test
	public void testControllerUpdateBestSeller() throws Exception {
		Laptop laptop = getLaptop();
		MvcResult result = this.mockMvc.perform(get("/updateBestSeller").param("id", laptop.getId().toString()))
				.andReturn();
		yakshaAssert(currentTest(),
				result.getModelAndView() != null && result.getModelAndView().getViewName() != null
						&& result.getModelAndView().getViewName().contentEquals("redirect:/laptop/list"),
